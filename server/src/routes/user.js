...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        if (response.rows.length === 0) {
            response = await insertNewUser(name, id, defaultProfilePic);
        }
...
router.post("/new-user", async (req, res) => {
    ...
    try {
        const response = await insertNewUser(name, firebase_uid, profile_pic);
        res.json(response.rows[0]);
    } catch (err) {
        console.error("ERROR: ", err);
        res.sendStatus(500);
...
async function insertNewUser(name, firebase_uid, profile_pic) {
    const query = {
